...
import { NeedBasedBehavior, NPCBehaviorState, NPCDecision } from './need-based-behavior';
...
export class DailyCycleSystem {
  ...
  getNPCsAtLocation(locationId: string): NPCBehaviorState[] {
    return Array.from(this.npcs.values())
      .filter(npc => npc.currentLocation === locationId);
  }
  ...
  private runCheckpoint(hour: number): void {
    ...
    console.log('\n🏃 === SECURITY CHECK ===');
    const fleeingNPCs = Array.from(this.npcs.values())
      .filter(npc => npc.needSafety < 20);
   ...
   */
  getNextCheckpoint(currentHour: number): { hour: number; name: string; hoursUntil: number } {
