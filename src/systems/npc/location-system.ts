...
export class LocationSystem {
  ...
  getAvailableHomes(): Location[] {
    return this.getLocationsByType('home')
      .filter(home => !home.capacity || (home.residents?.length || 0) < home.capacity);
  }
  ...
  logLocations(): void {
    ...
    // Social
    console.log('\n👥 SOCIAL LOCATIONS:');
    for (const social of [...byType.tavern, ...byType.market, ...byType.temple]) {
